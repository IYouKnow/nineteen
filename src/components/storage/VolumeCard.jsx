...
export default function VolumeCard({ volume, projects = [] }) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-semibold text-foreground">{volume.name}</p>
            <p className="truncate font-mono text-xs text-muted-foreground">
              {t.label} · {volume.size} GB
            </p>
          </div>
        </div>
      ...
      {volume.description && (
        <p className="mt-3 line-clamp-2 text-xs text-muted-foreground">{volume.description}</p>
      )}
