...
// giteaProfileURL builds a link to the user's profile on their Gitea instance
// from the integration's stored base_url, or "" when unavailable.
function giteaProfileURL(integration) {
  try {
    const cfg = JSON.parse(integration.config || "{}");
    const base = (cfg.base_url || "").trim().replace(/\/+$/, "");
    return base && integration.username ? `${base}/${integration.username}` : "";
  } catch {
    return "";
  }
}
          ...
          <div className="min-w-0 flex-1">
            ...
            {isConnected && integration.username && (
              <div className="flex items-center gap-2 mt-2">
                ...
                ) : integration.provider === "gitea" && giteaProfileURL(integration) ? (
                  <a
                    href={giteaProfileURL(integration)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-muted-foreground underline-offset-2 hover:text-foreground hover:underline"
                  >
